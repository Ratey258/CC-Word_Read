...
- **Phase 0：立即修复的小 Bug（高收益、低改动）**
  - 修正 `Editor.vue` 中自动开始阅读监听的绑定/解绑问题，确保只存在一个 `handleAutoStart` 引用，`addEventListener` / `removeEventListener` 成对。
  - 规范 `novelStore.isRestoringFromHistory` 的使用方式，统一通过 `ref` 或 store 内部方法修改，避免把 `Ref` 变成普通布尔值。
  - 核对 `UpdateChecker.vue` 与 Tauri 后端命令（`check_for_updates`、`download_and_install_update`）是否完全一致。

- **Phase 1：关键路径性能与健壮性**
  - 优化 `chapterParser.ts` 中章节偏移计算逻辑，用前缀和数组替代 `lines.slice(0, i).join('\n')`，将复杂度从 O(n²) 降到 O(n)。
  - 为章节解析与历史恢复补充少量关键单元测试，在后续结构重构前先把核心行为“钉住”。

- **Phase 2：可维护性与一致性**
  - 统一在 `Editor.vue`、`useEditorScroll.ts` 中使用 `UI.TITLEBAR_HEIGHT` / `UI.RIBBON_HEIGHT` 等常量，移除魔术数字。
  - 分步骤拆分 `Ribbon.vue`：先做逻辑分组，再提取纯展示子组件，最后将文件菜单等高复杂度区域独立为子组件，降低单文件复杂度。
